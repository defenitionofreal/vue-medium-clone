            ...
            <ul class="nav nav-pills outline-active">
              ...
              <li class="nav-item">
                <router-link
                  ...
                  class="nav-link"
                  :class="{active: routeName === 'profileFavorites'}"
                >
                  Favorites Posts
                </router-link>
              </li>
...
import {actionsType as userProfileActionTypes} from "@/store/modules/userProfile";
...
export default {
  ...
  computed: {
    ...
    // чтобы ссылка активная подсвечивалась
    routeName() {
      return this.$route.name
    }
  },
  // здесь watch следит за user slug в урл
  // если меняется то данные перезагружаются
  // (для баннера иконки)
  watch: {
    userProfileSlug() {
      this.getUserProfile()
